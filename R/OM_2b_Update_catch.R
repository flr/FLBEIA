...
#-------------------------------------------------------------------------------
CBaranov.CAA <- function(fleets, biols, BDs, biols.ctrl, fleets.ctrl, advice, year = 1, season = 1, flnm = 1, stknm = 1,...){
  ...
  for(i in 1: it){
    ...
    Cam[,,,i] <- CBaranov(E = eff[1,], fleets=fleets,biols=biols, Cr=Cr.f[i],N = Nst[,,i],   efs.m = efs.m[,i,drop=FALSE], q.m = q.m[,,,i,drop=FALSE], 
                          alpha.m = alpha.m[,,,i], beta.m = beta.m[,,,i], wd.m =wd.m [,,,i,drop=FALSE],
                          wl.m = wl.m[,,,i,drop=FALSE], ret.m = ret.m[,,,i,drop=FALSE],
                          tac=TAC.ss[i],Cayr_1=Cayr_1,Nayr_1=Nayr_1,Mayr_1=Mayr_1,Na=Na,Ma=Ma,Wa=Wa,Cafyr_1=Cafyr_1)
  }
  # Cam <- CobbDouglasAge(E = eff[1,], N = Nst, wl.m = wl.m, wd.m = wd.m, ret.m = ret.m, q.m = q.m,
  #                        efs.m = efs.m, alpha.m = alpha.m, beta.m = beta.m, rho = rho)
