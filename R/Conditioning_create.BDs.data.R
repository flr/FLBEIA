...
# NOTE #1:      Return a list with FLBDsim objects
###############################################################################
#-------------------------------------------------------------------------
#  inputs: 
#
...
#   stk_bd.model:   Name of the BD model (character)
...
#   stk_biomass.flq:  Biomass (FLQuant)
#   stk_catch.flq:    Catch (FLQuant)
#   stk_range.min       <- minimum age 
#   stk_range.max       <- maximum age
#   stk_range.plusgroup <- minimum age included in the oldest class group
#   stk_range.minyear   <- minimum year
#
# (optional)
...
#-------------------------------------------------------------------------------
#   Section 1:        Create FLBDSim for each stock
...
create.BDs.data <- function(path){  
  ...
  nmstks       <- unique(sub('.*?^(.*?)_bd.model*', '\\1', ls(pattern='_bd.model',envir=sys.frame(which=0))))
  ...
  if(n.stk.BD != 0){
    ...
    for(i in 1:n.stk.BD){
      ...
      params <- array(dim = c(stk.param.n,ny,ns,ni), 
                 dimnames = list(param=ac(1:stk.param.n),year = ac(first.yr:last.yr),ns=ac(1:ns), iter = 1:ni))
                      
      stk.bd <- FLBDsim(name =  nmstk, model=stk.model,biomass=flq.stk, catch=flq.stk, uncertainty=flq.stk,params=params)
      ...
      #-----------------------------------------------------------------------------
        ...
        if(!all(is.na(stk.uncertainty))){
          log.dim <- equal.flq.Dimnames(lflq=list(stk.uncertainty,stk.bd@uncertainty),2)
          if(!log.dim)stop('BD uncertainty dimension names \n')
        ...
        if(!all(is.na(stk.biomass))){
          log.dim <- equal.flq.Dimnames(lflq=list(stk.biomass,stk.bd@biomass[,hist.yrs]),2)
          if(!log.dim)stop('in BD biomass dimension names \n')
          if(!(any(dim(stk.biomass)[3]==c(1,stk.unit))))stop('in biomass number of stock units 1 or stk.unit')
          if(!(any(dim(stk.biomass)[4]==c(1,ns))))stop('in biomass number of seasons 1 or ns')
          if(!(any(dim(stk.biomass)[6]==c(1,ni))))stop('in biomass number of iterations 1 or ni')
        }else{cat('BD biomass values all NA-s \n')}
      
        if(!all(is.na(stk.catch))){
          log.dim <- equal.flq.Dimnames(lflq=list(stk.catch,stk.bd@catch[,hist.yrs]),2)
          if(!log.dim)stop('in BD catch dimension names \n')
          if(!(any(dim(stk.catch)[3]==c(1,stk.unit))))stop('in catch number of stock units 1 or stk.unit')
          if(!(any(dim(stk.catch)[4]==c(1,ns))))stop('in catch number of seasons 1 or ns')
          if(!(any(dim(stk.catch)[6]==c(1,ni))))stop('in catch number of iterations 1 or ni')
        }else{cat('BD catch values all NA-s \n')}
 
      log.dim <- equal.flq.Dimnames(lflq=list(stk.params,stk.bd@params),1:4)
      if(!log.dim)stop('BD parameters dimension names \n')
      ...
      }else{stop('BD parameters all NA-s \n')}
    ...
    #==============================================================================
      ...
      BDs     <-  sapply(stk.bds, get, envir=sys.frame(sys.parent(-1)), simplify=FALSE)
