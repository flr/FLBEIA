#-------------------------------------------------------------------------------
#                          HCRs
#   - CFPMSYHCR a HCR adapting the MAP HCR to allow flexibility in the year Fmsy is achieved. 
#   - You can specify the year in which you aim to reach Fmsy, with a linear transition between
#     Fsq to Fmsy in the intervening years
#     i.e. if target year is 2020, then for the current year y,
...
#-------------------------------------------------------------------------------
# CFPMSYHCR(stocks, covars, advice, hcr.ctrl)   
...
#    nyears      <- ifelse(is.null(advice.ctrl[[stknm]][['nyears']]), 3, advice.ctrl[[stknm]][['nyears']]) 
   ...
   # disc.nyears  <- ifelse(is.null(advice.ctrl[[stknm]][['disc.nyears']]), wts.nyears, advice.ctrl[[stknm]][['disc.nyears']]) 
    ...
    Yrtg <- advice.ctrl[[stknm]][['ref.pts']]['Yrtg',year+1,]
   ...
   # else if Fadv tq SSB[tac year + 1] > SSB[data year] + (Bpa - SSB[data year])/K, if this leads to and F > Ftarget => Fadv = Farget
    ...
    for(i in 1:iter){
      ...
      # Here we add the new calculation of Ftg dependent on Yrtg
      # When assyrnumb+1 >= Yrtg, Ftg=Ftg
      # When assyrnumb+1 < Yrtg, F is the linear difference between Fsq and Ftg
      
      Ftg2<-ifelse(assyrnumb+1>=Yrtg,Ftg,((1-(1/(Yrtg-assyrnumb+1)))*fsq)+((1/(Yrtg-assyrnumb+1))*as.numeric(Ftg)))
      #print(Ftg2)
