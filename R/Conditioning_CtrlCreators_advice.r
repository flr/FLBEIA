...
create.advice.ctrl <- function(stksnames, HCR.models = NULL, HCR.ctrls = NULL){
    ...
    if(is.null(advice.models)) advice.models <- rep('fixedAdvice', length(stksnames))
    ...
    for(st in 1:length(stksnames)){
        ...
        res[[st]] <- call(HCRmodcreator, res = res[[st]], stkname = st, args = extra.args)
    }
...
create.annualTAC.ctrl <- function(resst,stkname, largs){
    ...
    if(is.null(Ftarget.stk)) warning("Ftarget for stock, '", stkname,"' has not been specified in argument: ", paste("Ftarget",stkname,sep = "."), ".\n - A fwd.ctrl element with empty Ftarget will be created.\n - FILL IT BY HAND!!!!")
