...
SCD <- function(fleets, covars, fleets.ctrl, flnm, advice, year = 1, season = 1,...){
    ...
    # VaC
    VaC <- seasonSums(totvcost_flbeia(fleet = fleet, advice = advice, fleet.ctrl = fleet.ctrl, covars = covars)[,year]) # total annual variable costs
    # FxC
    ...
    # Revenue
    Rev <- revenue_flbeia(fleet, fleet.ctrl, advice)[,year]
