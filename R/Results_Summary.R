...
bioSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] <= 6){ # the object is in long format
    ...
    res <- aggregate(list(biomass = obj$biomass,catch = obj$catch,catch.iyv = obj$catch.iyv, 
                       ...
                       list(stock = obj$stock, year = obj$year, scenario = obj$scenario), quantile, prob = prob, na.rm=F)
...
fltSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] < 10){ # the object is in long format
    ...
    if(!('season' %in% names(obj))){
      res <- aggregate(value ~ fleet + indicator + year + scenario, obj, quantile, prob = prob,na.rm=F)
      ...
      res <- aggregate(value ~ fleet + indicator + year + scenario + season, obj, quantile, prob = prob, na.rm=F)
    ...
    if(!('season' %in% names(obj))){
      ...
      names(res)[-c(1:3)] <- unlist(mget(paste('nms', 1:19, sep="")))
        ...
        res <- aggregate(list(capacity = obj$capacity,      catch = obj$catch,         costs = obj$costs,          discards = obj$discards,       
                              ...
                              price  = obj$price,           grossSurplus  = obj$grossSurplus,    quotaUpt  = obj$quotaUpt,   salaries  = obj$salaries, 
                              vcosts  = obj$vcosts,         profitability  = obj$profitability), 
                         list(fleet = obj$fleet, year = obj$year, season = obj$season, scenario = obj$scenario), quantile, prob = prob, na.rm = F)
...
fltStkSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] < 10){ # the object is in long format
    ...
    if(!('season' %in% names(obj))){
      res <- aggregate(value ~ fleet + stock + indicator + year + scenario, obj, quantile, prob = prob,na.rm=F)
      res <- cbind(res[,1:5], data.frame(res[,6]))
      ...
      res <- aggregate(value ~ fleet + stock + indicator + year + scenario + season, obj, quantile, prob = prob, na.rm=F)
    ...
    if(!('season' %in% names(obj))){
      ...
      res <- aggregate(list(catch = obj$catch, discards = obj$catch, discRat = obj$discRat, landings = obj$landings,       
                            price = obj$price, quota = obj$quota,    quotaUpt = obj$quotaUpt),                   
                       list(fleet = obj$fleet, stock = obj$stock, year = obj$year, season = obj$season, scenario = obj$scenario), quantile, prob = prob, na.rm = FALSE)
...
mtStkSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] < 10){ # the object is in long format
    ...
    if(!('season' %in% names(obj))){
      res <- aggregate(value ~ fleet + metier + stock + indicator + year + scenario, obj, quantile, prob = prob,na.rm=FALSE)
      res <- cbind(res[,1:6], data.frame(res[,7]))
      ...
      res <- aggregate(value ~ fleet + metier + stock + indicator + year + scenario + season, obj, quantile, prob = prob, na.rm=F)
    ...
    if(!('season' %in% names(obj))){
      ...
      res <- aggregate(list(catch = obj$catch, discards = obj$discards, discRat = obj$discRat, landings = obj$landings,       
                            price = obj$price),                   
                       list(fleet = obj$fleet, metier = obj$metier, stock = obj$stock, year = obj$year, season = obj$season, scenario = obj$scenario), quantile, prob = prob, na.rm = FALSE)
...
mtSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] < 9){ # the object is in long format
    ...
    if(!('season' %in% names(obj))){
      res <- aggregate(value ~ fleet + metier + indicator + year + scenario, obj, quantile, prob = prob,na.rm=F)
      res <- cbind(res[,1:5], data.frame(res[,6]))
      ...
      res <- aggregate(value ~ fleet + metier + indicator + year + scenario + season, obj, quantile, prob = prob, na.rm=F)
      res <- cbind(res[,1:6], data.frame(res[,7]))
    ...
    if(!('season' %in% names(obj))){
      ...
      res <- aggregate(list(effort = obj$effort,  effshare = obj$effshare, vcost = obj$vcost, grossValue = obj$grossValue,       
                            vcost = obj$vcost),                    
                       list(fleet = obj$fleet, metier = obj$metier,  year = obj$year, season = obj$season, scenario = obj$scenario), quantile, prob = prob, na.rm = FALSE)
...
advSumQ <- function(obj,  prob = c(0.95,0.5,0.05)){
  ...
  if(dim(obj)[2] < 8){ # the object is in long format
    ...
    res <- aggregate(value ~  stock + indicator + year + scenario, obj, quantile, prob = prob,na.rm=F)
    res <- cbind(res[,1:4], data.frame(res[,5]))
...
npvQ <- function(obj, prob = c(0.05,0.5,0.95)){
  ...
  res <- aggregate(npv ~  fleet + scenario, obj, quantile, prob = prob,na.rm=F)
...
if(dim(obj)[2] < 10){ # the object is in long format
  ...
  if(!('season' %in% names(obj))){
    ...
    res <- aggregate(list(    catch = obj$catch,         costs = obj$costs,          discards = obj$discards,       
                          ...
                          price  = obj$price,           grossSurplus  = obj$grossSurplus,    quotaUpt  = obj$quotaUpt,   salaries  = obj$salaries, 
                          vcosts  = obj$vcosts,         profitability  = obj$profitability), 
                     list(fleet = obj$fleet, year = obj$year, season = obj$season, scenario = obj$scenario), quantile, prob = prob, na.rm = FALSE)
