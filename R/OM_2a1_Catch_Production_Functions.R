...
#   
#   
#       gamma <- uniroot(find_gamma, interval=c(-1e-12,5), Fayr_1 = Fayr_1, N = N, M = M, W = Wall, tac = tac,
#                    tol = 1e-12, extendInt = "yes")$root
#   
#       # F corresponding to tac in year [y], using the selection pattern in [y-1].
#       Fayr <- Fayr_1*gamma
#       
#       # F corresponding to the quota share of our fleet, that need to be discounted, before calculating the real one.
#       # calculate it using theoretical partial F, i.e, the ration between the quota share of the fleet and the
#       Fafyr.   <- Fayr*Cr/tac 
#       Fafyr_1[Cyr_1==0] <- 0
#   
#   
...
#       N <- array(rep(N, each = dim(efs.m)[1]),dim=c(dim(Ef)))
# 
#       C.m <- q.m*Ef/z_new*(1-exp(-z_new))*N*W
#   
#       Clim <- sweep(W*N,4,rho,"*")
#   
#       C.m <- ifelse(C.m < Clim, C.m, Clim) # The truncation  of production function is applied at metier level.
#   
#       return(catch =  C.m)}  # [mt,na,nu,it]
#   
...
Baranov   <- function(E, Cr, efs.m, wl.m, wd.m, ret.m, q.m,
  ...
  if(Fknown == FALSE){
    ...
    ## Calculate Fa in year [yr-1] Fayr_1 for the whole fleet.
    findF <- function(Fa,C, M, N){
      C. <- (Fa/(Fa+M))*(1-exp(-M-Fa))*N
      res <- sum(C.)-sum(C)
      return(res)
    ...
    ## The partial fishing mortality of the fleet in yr-1
    Fafyr_1 <- Fayr_1*Cfyr_1/Cyr_1
    # eliminate the possible NA-s derived from total catch = 0.
    Fafyr_1[Cyr_1==0] <- 0
    
    # The partial fishing mortality of the rest of the fleet.
    Fapyr_1 <- Fayr_1 - Fafyr_1
      

    # To estimate Fa, total fishing mortality in year [y], estimate gamma where Fa = Fayr-1 *gamma + q*E
    #if(na>1) Na  <-  as.vector(N)*iter(exp((biols[[stknm]]@m/2)[,yr,,ss]),it) # N is in the middle of the season,

    find_gamma <- function(gamma, Fapyr_1, Faf, N,M,W,Cr,tac){
      
      Fa  <- Fapyr_1*gamma + Faf 
      z <- Fa + M
      
      # The catch of our fleet
      Cf.  <- (Faf/z) *(1-exp(-z))*N*W
      
      # The catch of the rest of the fleet
      C.  <- (Fapyr_1*gamma/z) *(1-exp(-z))*N*W
      
      # The catch of the rest of the fleet equalt to the rest of the tac. 
      Cest <- tac - Cr
    ...
    Faf <- array(apply(q.m*Ef,2:4, sum), dim = dim(q.m)[-1])
    
    gamma <- uniroot(find_gamma, interval=c(0,5), Fapyr_1 = Fapyr_1, Faf = Faf, N = N, M = M, W = Wall, tac = tac,  Cr = Cr,
                   tol = 1e-12, extendInt = "yes")$root
      
      
    # F corresponding to Cest in year [y], using the selection pattern in [y-1].
    Fap <- Fayr_1*gamma
    Fa  <- Fayr_1*gamma + Faf
    ...
    z <- Fa+M
    z <- array(z,dim=c(dim(Ef)))
    
    N <- array(rep(N, each = dim(efs.m)[1]),dim=c(dim(Ef)))

    C.m <- Faf/z*(1-exp(-z))*N*W
